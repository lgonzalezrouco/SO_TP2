...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    ...
    printf(
	"Written by Ian Seyler @ Return Infinity (ian.seyler@returninfinity.com)\n\n");
...
int initialize(char *diskname, char *size, char *mbr, char *boot,
	       char *kernel) {
  ...
  for (i = 0; size[i] != '\0' && ret == 0; ++i) {
    ...
    } else if (i == 0)	// No digits specified
    {
      ...
      switch (toupper(ch)) {
	case 'K': diskSizeFactor = 1; break;
	case 'M': diskSizeFactor = 2; break;
	case 'G': diskSizeFactor = 3; break;
	case 'T': diskSizeFactor = 4; break;
	case 'P': diskSizeFactor = 5; break;
	default:
  ...
  if (ret == 0) {
    ...
    while (writeSize < diskSize) {
      ...
      percent /= diskSize;
      percent *= 100;
      printf("Formatting disk: %llu of %llu bytes (%.0f%%)...\r", writeSize,
	     diskSize, percent);
...
void create(char *filename, unsigned long long maxsize) {
  ...
  if (findfile(filename, &tempentry, &slot) == 0) {
    ...
    for (tint = 0; tint < num_used_entries + 1; tint++) {
      ...
      // and the beginning of the current file (or the last data block if there
      // are no more files).
...
void read(char *filename) {
  ...
  if (0 == findfile(filename, &tempentry, &slot)) {
	...
	putc(getc(disk), tfile);  // This is really terrible.
				  // TODO: Rework with fread and fwrite (ideally
				  // with a 2MiB buffer)
      }
      fclose(tfile);
      printf("Complete\n");
...
void write(char *filename) {
  ...
  if (0 == findfile(filename, &tempentry, &slot)) {
    printf(
	"Error: File not found in BMFS. A file entry must first be created.\n");
  } else {
	...
	for (tint = 0; tint < tempfilesize; tint++) {
	  putc(getc(tfile), disk);  // This is really terrible.
				    // TODO: Rework with fread and fwrite
				    // (ideally with a 2MiB buffer)
	}
	// Update directory
