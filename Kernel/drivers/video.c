...
struct vbe_mode_info_structure {
	uint16_t attributes;   // deprecated, only bit 7 should be of interest to you, and
...
void printNewline(void) {
	...
	if (_Y + 2 * _charHeight <= _screenData->height) {
		...
		memset(getPtrToPixel(0, _screenData->height - _charHeight),
		       0,
	...
	if (c >= FIRST_CHAR && c <= LAST_CHAR) {
		...
		for (int h = 0; h < _charHeight; h++) {  // Iteracion por filas
			...
			for (uint8_t i = 0; i < _charWidth; i++) {  // Iteracion por columnas
				...
				if (mask & 0b10000000) {
					mask = 0b00000001;
					data++;  // Pasa al siguiente byte dentro de una fila o de la
					         // siguiente
				} else {
		...
		if (*fmtPtr == '%') {
			...
			switch (*fmtPtr) {
				...
				case 'x':
					...
					break;
				case 's':
					printNChars(' ', dx);  // A diferencia %x y %d, la cantidad de
					                       // espacios es igual al numero
