...
void *schedule(void *currentSP) {
    ...
    if (quantumRemaining > 0 && currentProcess->status != ZOMBIE && currentProcess->status != BLOCKED &&
...
void stopProcess(void *currentSP) {
    ...
    if (currentProcess->status == RUNNING) {
        currentProcess->status = READY;
        currentProcess->priority =
            currentProcess->priority > 1 ? currentProcess->priority - 1 : currentProcess->priority;
...
int killProcess(uint16_t pid, int retValue) {
    ...
    if (process->status == ZOMBIE || process->pid == IDLE_PID || process->pid == SHELL_PID)
        return INVALID_PROCESS;
    ...
    // El proceso killeado se agrega a la lista de zombies del padre y si el padre hizo waitpid de este, se desbloquea
