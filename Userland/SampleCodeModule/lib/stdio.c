  ...
  while (*fmtPtr && buffer[bIdx] && bIdx < MAX_CHARS) {
	...
	case 'c':
	  *(char *)va_arg(v, char *) = buffer[bIdx];
	  ...
	  break;
	case 'd': *(int *)va_arg(v, int *) = strtoi(&buffer[bIdx], &end); break;
	case 'x': *(int *)va_arg(v, int *) = strtoh(&buffer[bIdx], &end); break;
	case 's':
	  end = &buffer[bIdx] +
