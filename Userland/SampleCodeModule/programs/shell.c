...
static Command commands[] = {
    ...
    {"inforeg", "Informacion de los registros que fueron caputrados en un momento arbitrario de ejecucion del sistema",
...
static void help() {
    for (int i = 0; i < QTY_COMMANDS; i++) printf("%s: %s\r\n", commands[i].name, commands[i].description);
}
...
static void printInfoReg() {
    ...
    for (int i = 0; i < len; i++) printf("%s: 0x%x\n", _regNames[i], regSnapshot[i]);
}
